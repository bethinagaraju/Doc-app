...
import { NativeStackNavigationProp } from '@react-navigation/native-stack';
import tw from 'twrnc';
import { ArrowLeft, UserPlus, Users, ChevronRight } from 'lucide-react-native';

type RootStackParamList = {
  // Define your routes here, for example:
  AddDoctor: undefined;
  ViewDoctors: undefined;
};
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-green-50`}>
      ...
      <View style={tw`bg-green-600 p-4 flex-row items-center shadow-md`}>
        ...
        <View>
          <Text style={tw`text-white text-xl font-bold`}>Doctor Management</Text>
          <Text style={tw`text-green-100 text-sm`}>Select an option below</Text>
        </View>
      </View>

      {/* Main Content - Rectangle Cards */}
      <View style={tw`flex-1 p-6 justify-center gap-6`}>

        {/* Card 1: Add Doctor */}
        <TouchableOpacity
          style={tw`bg-white p-6 rounded-xl shadow-sm border-l-8 border-green-500 flex-row items-center justify-between h-32`}
          onPress={() => {
            navigation.navigate('AddDoctor'); // Navigate to Add Doctor Screen
          }}
        >
          <View style={tw`flex-row items-center`}>
            <View style={tw`bg-green-100 p-4 rounded-full mr-4`}>
              <UserPlus size={32} color="#15803d" />
            </View>
            <View>
              <Text style={tw`text-xl font-bold text-gray-800`}>Add Doctor</Text>
              <Text style={tw`text-gray-500 text-sm mt-1`}>Register new staff</Text>
            </View>
          </View>
        ...
        {/* Card 2: View Doctors */}
        <TouchableOpacity
          style={tw`bg-white p-6 rounded-xl shadow-sm border-l-8 border-blue-500 flex-row items-center justify-between h-32`}
          onPress={() => {
            navigation.navigate('ViewDoctors'); // Navigate to View Doctors Screen
          }}
        >
          <View style={tw`flex-row items-center`}>
            <View style={tw`bg-blue-100 p-4 rounded-full mr-4`}>
              <Users size={32} color="#1d4ed8" />
            </View>
            <View>
              <Text style={tw`text-xl font-bold text-gray-800`}>View Doctors</Text>
              <Text style={tw`text-gray-500 text-sm mt-1`}>Manage existing staff</Text>
            </View>
          </View>
          <ChevronRight size={24} color="#9ca3af" />
        </TouchableOpacity>
