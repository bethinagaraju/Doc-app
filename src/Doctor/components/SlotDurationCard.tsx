...
export default function SlotDurationCard({
    selected,
    onChange,
}: Props) {
    return (
        <View
            style={tw`bg-white border border-[#DAE1E7] rounded-2xl p-4 shadow-sm`}
        >
            {/* Header */}
            <View>
                <TimerIcon />
                ...
                <Text
                    style={tw`mt-2 text-[20px] font-semibold text-[#011D35]`}
                >
                    Slot Duration
                </Text>

                <Text
                    style={tw`text-sm text-[#434653] mt-1`}
                >
            ...
            <View style={tw`flex-row mt-4`}>
                {durations.map((duration) => {
                    const active = duration === selected;
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.8}
                            style={tw.style(
                                "px-4 py-2 rounded-full border mr-2 min-w-[62px] items-center justify-center",
                                active
                                    ? "bg-[#124CB8] border-[#124CB8]"
                                    : "bg-white border-[#124CB8]"
                            )}
                        >
                            <Text
                                style={tw.style(
